...
_distance = 5; // Distance from object to display Refuel Message
_amount= 0.002; // Amount of fuel to add per loop. Default was 0.005
...
while {true} do
{
    if (!isNull player) then {
        ...
        if (_vehicle != _currentVehicle) then {
            ...
            if (_currentVehicle != player && _isNearFeed && !(_currentVehicle isKindof "Bicycle") && !(_currentVehicle isKindof "Air")) then {  //change "Bicycle" to "Land" to allow only air vehicles to aut-refuel
                _vehicle = _currentVehicle;
 
                _vehicle_refuel_id = _vehicle addAction [("<t color=""#FFBF00"">" + ("Refuel (Unpowered)")+ "</t>"), "skaronator\refuel\vehicle_refuel.sqf", [_amount], -1, false, true, "", "vehicle _this == _target && local _target"];
            };
        };
