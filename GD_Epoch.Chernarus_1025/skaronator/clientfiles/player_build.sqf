...
_isTent = (_classname == "TentStorage");
_isTentStorageDomed = (_classname == "TentStorageDomed");
_isTentStorageDomed2 = (_classname == "TentStorageDomed2");
_isLockboxStorageLocked = (_classname == "LockboxStorageLocked"); 
_isGeneratorDZ = (_classname == "Generator_DZ");
_isLandFireDZ = (_classname == "Land_Fire_DZ");

_distance = 100; // Distance being able to build around a Plot Pole - Default 30
_needText = "Plot Pole";

if(_isPole) then {
	_distance = 150; // required distance to next PlotPole - Default 45
};
...
if(_isPole and _IsNearPlot > 0) exitWith {  TradeInprogress = false; cutText ["Cannot build plot pole within 150m of an existing plot." , "PLAIN DOWN"]; };
//diag_log format["DEBUG BUILDING _IsNearPlot: %1", _IsNearPlot];
if(_IsNearPlot == 0) then {
	if(_isPole or _isWorkBench or _isTent or _isTentStorageDomed or _isTentStorageDomed2 or _isGeneratorDZ or _isLockboxStorageLocked or _isLandFireDZ) then {
	...
	if (dayz_playerUID == _ownerID) then {
		...
		diag_log format["DEBUG BUILD:(%1 IN %2)", _ownerID, _friendlies];
		//"DEBUG BUILD:(136186054 IN ["709"])"
...
// _message
if(!_canBuildOnPlot) exitWith {  TradeInprogress = false; cutText [format["You need a %1 nearby!",_needText] , "PLAIN DOWN"]; };
