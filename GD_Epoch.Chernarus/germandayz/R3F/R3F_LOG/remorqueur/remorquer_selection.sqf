...
else
{
	...
	if (!(isNull _objet) && (alive _objet) && !(_objet getVariable "R3F_LOG_disabled")) then
	{
		...
		{
			if (_objet distance _remorqueur <= 15) then
			{
                ...
                if(_countTransportedBy + _countTowedVehicles <= 2) then
                {
					...
					_objet attachTo [_remorqueur, [
						0,
						(boundingBox _remorqueur select 0 select 1) + (boundingBox _objet select 0 select 1) +0, //vehicle tow distance (default +3) +10 ends in pushing -Star
