    ...
    private["_body","_result"];
        ...
        _model = typeOf _body;
	// Aenderung anti Duping - Andro		
	if( _model in ["Survivor2_DZ", "SurvivorW2_DZ"] ) then {
	cutText ["The clothing is not interesting for you.", "PLAIN DOWN"];
	};	
// -------------------- Aenderung Andro damit eine maennliche Leiche einen meannlichen Corps erzeugt
	if( _model in ["Bandit1_DZ","Bandit2_DZ","Soldier_Crew_PMC","Sniper1_DZ","Camo1_DZ","Soldier1_DZ","Rocket_DZ","Rocker1_DZ","Rocker2_DZ","Rocker3_DZ","Rocker4_DZ","Priest_DZ","Functionary1_EP1_DZ","GUE_Commander_DZ","Ins_Soldier_GL_DZ","Haris_Press_EP1_DZ","Pilot_EP1_DZ","RU_Policeman_DZ","Soldier_TL_PMC_DZ","Soldier_Sniper_PMC_DZ","Soldier_Bodyguard_AA12_PMC_DZ","Drake_Light_DZ","CZ_Special_Forces_GL_DES_EP1_DZ","TK_INS_Soldier_EP1_DZ","TK_INS_Warlord_EP1_DZ","FR_OHara_DZ","FR_Rodriguez_DZ","CZ_Soldier_Sniper_EP1_DZ","Graves_Light_DZ","GUE_Soldier_MG_DZ","GUE_Soldier_Sniper_DZ","GUE_Soldier_Crew_DZ","GUE_Soldier_CO_DZ","GUE_Soldier_2_DZ"] ) then { _model = "Skin_" + _model;
            _rnd = random 1;
            if (_rnd > 0.4) then {
                    ...
                    _result = [player,_model];// call BIS_fnc_invAdd; Aenderung Andro damit kein Civ Clothing Spawnt
					player removeMagazine "ItemTrashRazor";
                    cutText ["As you try to remove the clothes, your razor broke.", "PLAIN DOWN"];
            };
...
// -------------------- Aenderung Andro - damit eine weibliche Leiche einen weiblichen Corps erzeugt
	if( _model in ["SurvivorWcombat_DZ","SurvivorWdesert_DZ","SurvivorWurban_DZ","SurvivorWsequishaD_DZ","SurvivorWsequisha_DZ","SurvivorWpink_DZ","SurvivorW3_DZ","BanditW1_DZ","BanditW2_DZ"] ) then { _model = "Skin_" + _model;	
            _rnd = random 1;
            if (_rnd > 0.4) then {
                    _result = [player,_model] call BIS_fnc_invAdd;
            }
            else {
                    _model = "Skin_SurvivorW2_DZ";
					_result = [player,_model];// call BIS_fnc_invAdd; Aenderung Andro damit kein Civ Clothing Spawnt
					player removeMagazine "ItemTrashRazor";
                    cutText ["As you try to remove the clothes, your razor broke.", "PLAIN DOWN"];
            };
